function Get-AzSecurityReport {
    <#
    .SYNOPSIS
        This script is designed to export the security score from Azure Security Center. Including:
        - The number of resources that are healthy, unhealthy, and are excluded
        - The weight, score and potential score for each categories
    ...
    process {
        ...
        #----------------------------------------------------------------------------------------
        # Module to get security score from subscription
        #----------------------------------------------------------------------------------------
        ...
        function Get-AzSecurityScore {
          ...
          param()
          process {
            $scoreControls = Get-AzSecuritySecureScoreControl
            $subName = (Get-AzContext | select -ExpandProperty Name).Split('(')[0]

            foreach ($control in $scoreControls) {
              $name = $control.DisplayName
              $weight = $control.Weight
              $score = $control.CurrentScore
              $maxScore = $control.MaxScore
              $percentage = $control.Percentage
              $healthy = $control.HealthyResourceCount
              $unhealthy = $control.UnhealthyResourceCount
              $na = $control.NotApplicableResourceCount
        ...
        foreach ($azSub in $selectedAzSubs) {
            ...
            Write-Host "Checking for security score in sub: $azSubName" -ForegroundColor green
            $subscriptionScore = Get-AzSecurityScore
        ...
        $score | Export-Excel -Path $excelPath -WorksheetName "SecurityScore"
