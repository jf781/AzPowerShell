function Import-AzPublicDnsZoneRecords {
  [CmdletBinding()]
  param(    [Parameter(
      Mandatory = $true,
  ...
  process {
    ...
    function New-PublicZoneDnsRecords {
      [CmdletBinding()]
      param(
        [Parameter(
          Mandatory = $true,
      ...
      process {
        ...
        foreach ($zone in $zones) {
          ...
          $dnsZone = Get-AzDnsZone | Where-Object -Property Name -EQ $zone.Name
      ...
      process {
        ...
        switch ($dnsRecord.RecordType) {
          A {
            ...
            $recordConfig += New-AzDnsRecordConfig -Ipv4Address $dnsRecord.Destination
          }

          AAAA {
            Write-Verbose "Record type is an AAAA record"
            $recordConfig += New-AzDnsRecordConfig -Ipv6Address $dnsRecord.Destination
          }

          CNAME {
            Write-Verbose "Record type is a CNAME record"
            $recordConfig += New-AzDnsRecordConfig -Cname $dnsRecord.Destination
          }

          MX {
            Write-Verbose "Record type is a MX record"
            $recordConfig += New-AzDnsRecordConfig `
          ...
          PTR {
            ...
            $recordConfig += New-AzDnsRecordConfig -Ptrdname $dnsRecord.Destination
          }

          TXT {
            Write-Verbose "Record type is a TXT record"
            $recordConfig += New-AzDnsRecordConfig -Value $dnsRecord.Destination
          }

          NS {
            Write-Verbose "Record type is a NS record"
            $recordConfig += New-AzDnsRecordConfig -nsdname $dnsRecord.Destination
          }

          CAA {
            Write-Verbose "Record type is Certificate Authority Authorization"
            $recordConfig += New-AzDnsRecordConfig `
              -CaaFlags $dnsRecord.CaaFlags `
              -CaaTags $dnsRecord.CaaTag `
              -CaaValue $dnsRecord.CaaValue
          }
    ...
    New-PublicZoneDnsRecords -csvFile $csv
