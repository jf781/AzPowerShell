function Get-AzRbacPermissions {
    <#
    .SYNOPSIS
        This script is designed to export the RBAC role assignments for management groups, subscritpions, and resource groups associated with the Azure tenant that it is ran against. 
    .DESCRIPTION
    ...
    .OUTPUTS
        It will output a CSV file that contains the following fields
        - Subscription Name
        - Scope of role assignment
        - Who the role is assigned to (Both Display Name and Sign In Name)
        - The object type the role is assigned to (User/Group/Service Principal/etc)
    ...
    If ($env:HOME) {
        ...
        Write-Host "Finished exporting RBAC Role Assingments. Export is saved to $env:HOME/Desktop/Azure-RBAC-Output-$Date.csv "
    }
    else {
