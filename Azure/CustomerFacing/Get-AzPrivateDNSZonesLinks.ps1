function Get-AzPrivateDNSZonesLinks {
  <#
  .SYNOPSIS
  ...
  process {
      ...
      function Get-PrivateDnsZones {
        ...
        foreach ($zone in $dnsZones) {
            ...
            # If no VNet links, add the zone to the array
            $props = [ordered]@{
                "SubscriptionName"              = (Get-AzContext).Subscription.Name
                "ResourceGroupName"             = $zone.ResourceGroupName
                "ZoneName"                      = $zone.Name
                "NumberOfVnetLinks"             = $zone.NumberOfVirtualNetworkLinks
                "NumberOfVnetRegistrationLinks" = $zone.NumberOfVirtualNetworkLinksWithRegistration
            }
      ...
      foreach ($azSub in $selectedAzSubs) {
        ...
        Write-Host "Getting Zones for for sub: $azSubName" -ForegroundColor green
        $subDnsZones = Get-PrivateDnsZones 
        
        $dnsZones.Add($subDnsZones) | Out-Null
        
      }

      $excelPath = Get-DesktopPath -date $date -fileName "AzPrivateDnsZones"
      ...
      $dnsZones | Export-Excel -Path $excelPath -WorksheetName "PrivateDnsZones"
