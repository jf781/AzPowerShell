function Create-AzSpecificTagReport {
  <#
    .SYNOPSIS
        This script is designed to export a list of resources and not if they have a specific tag defined:
        - Resources
        - Tags associated with each resource
    ...
    .INPUTS
        It will require the TagName parameter to be populated.  If you are not connected to Azure it will prompt you to login.

    .PARAMETER TagName
        You will need to specify the name of the tag that you will be reviewing each resource for.  
  ...
  process {
    ...
    #----------------------------------------------------------------------------------------
    # Module to get tags from Subscription
    #----------------------------------------------------------------------------------------
      ...
      process {
        ...
        try {
          foreach ($resource in $resources) {
            ...
            $resourceTagKeys = $resource.Tags.Keys
            
            if($resource.Tags){
              if($resource.Tags.ContainsKey($tagName)){
                foreach ($key in $resourceTagKeys) {
                  if($key -eq $tagName){
                    $tagValue = $null
                    $resource.tags.TryGetValue($key, [ref]$tagValue) | Out-Null
          ...
          Write-Verbose "Error checking for tags on $resourceId"
          Write-Host "Error checking for tags on resource: $resourceId" -ForegroundColor Red
          Write-Host "Error Msg: $_" -ForegroundColor Red
        }
    ...
    $installedModules = Confirm-ModulesInstalled -modules Az.Resources, ImportExcel
    $modulesNeeded = $False
    ...
    foreach ($azSub in $selectedAzSubs) {
      ...
      Write-Host "Checking for tags resources in sub: $azSubName" -ForegroundColor green
      $subscriptionTags = Get-AzTagReport -tagName $TagName
    ...
    $tags | Export-Excel -Path $excelPath -WorksheetName "Tags"
