function Get-AzPolicyAssignmentDetails {
  <#
  .SYNOPSIS
  ...
  process {
      ...
      Function Get-PolicyAssignmentDetails ($subId, $subDisplayName){
        $uri = "https://management.azure.com/subscriptions/" + $subId + "/providers/Microsoft.Authorization/policyAssignments?api-version=2022-06-01"
        $rawAssignments = Invoke-AzRestMethod `
        -Method GET `
        -Uri $uri
      
        $assignments = $rawAssignments.content | ConvertFrom-Json
      
        foreach($assignment in $assignments.value){
      
          # Get Assignment Definition Details
          if($assignment.properties.policyDefinitionId -like "*managementGroups*"){
            $mgmtGroupName = ($assignment.properties.policyDefinitionId).split("/")[4]
            $policyId = ($assignment.properties.policyDefinitionId).split("/")[-1]
            $definitionScope = "Management Group"
            $definitionScopeName = $mgmtGroupName
      
            $uri = "https://management.azure.com/providers/Microsoft.Management/managementGroups/" + $mgmtGroupName + "/providers/Microsoft.Authorization/policyDefinitions/" + $policyId + "/?api-version=2021-06-01"
            $rawDefinition = Invoke-AzRestMethod `
            -Method GET `
            -URI $uri
            
          }elseif($assignment.properties.policyDefinitionId -like "*subscription*"){
            $sub = ($assignment.properties.policyDefinitionId).split("/")[2]
            $policyId = ($assignment.properties.policyDefinitionId).split("/")[-1]
            $definitionScope = "Subscription"
            $definitionScopeName = $sub
      
            $uri = "https://management.azure.com/subscriptions/" + $sub + "/providers/Microsoft.Authorization/policyDefinitions/" + $policyId + "?api-version=2021-06-01"
            $rawDefinition = Invoke-AzRestMethod `
            -Method GET `
            -URI $uri$
            ...
            $policyId = ($assignment.properties.policyDefinitionId).split("/")[-1]
            $definitionScope = "Built In"
            $definitionScopeName = ""
      
            $uri = "https://management.azure.com/providers/Microsoft.Authorization/policyDefinitions/" + $policyId + "?api-version=2021-06-01"
            $rawDefinition = Invoke-AzRestMethod `
            -Method GET `
            -URI $uri$
          ...
          $definition = $rawDefinition.Content | ConvertFrom-Json
          
          # Get Assignment Description
          $assignmentDesc = $assignment.properties.description
          if ($assignmentDesc){
            Write-Verbose "`$assignmentDesc valid"
          }else{
            $assignmentDesc = "No description on assignment"
          }
      
          # Get Assignment Display Name
          $assignmentDisplayName = $assignment.properties.displayName
          if ($assignmentDisplayName){
            Write-Verbose "`$assignmentDisplayName valid"
          }else{
            $assignmentDisplayName = ($assignment.id).split("/")[-1]
          }

          # Export details about assignment
          $props = [ordered]@{
            ...
            AssignmentEnforcementMode = $assignment.properties.enforcementMode
            AssignmentCreatedOn       = $assignment.systemData.createdAt
            AssignmentLastModified    = $assignment.systemData.lastModifiedAt
            Definition                = $definition.properties.DisplayName
            DefinitionId              = $assignment.properties.policyDefinitionId
            DefinitionType            = $definition.properties.policyType
      ...
      foreach ($azSub in $selectedAzSubs) {
          ...
          Write-Host "Getting Policy Assignments for sub: $azSubName" -ForegroundColor green
          $subAssignments = Get-PolicyAssignmentDetails -subId $azSubId -subDisplayName $azSubName
          $assignments += $subAssignments
      }

      $excelPath = Get-DesktopPath -date $date -fileName "Azure-Policies"
      ...
      $assignments | Export-Excel -Path $excelPath -WorksheetName "Assignments"
