Function Test-AzResourceMigration {
  ...
  #>
    ...
    $validatedOutputPath = Confirm-OutputPath -path $outputPath
    ...
    if ($validatedOutputPath) {
      ...
      foreach ($resource in $resources) {
        ...
        if ($requestStatus.dependencies -ne $null) {
          Write-Verbose "In If, `$requestStatus.dependencies is not null.  `Adding to $dependencyReport"
          $dependencyReport += $requestStatus.dependencies
        }
        else {
          Write-Verbose "In Else, `$requestStatus.dependencies is null."
        }
    ...
    try {
      Write-Verbose "Attempting to write date to `$validatedOutputPath: $validatedOutputPath"
      $subscriptionName = ((Get-AzContext).Subscription.Name -replace ' ', '-' `
          -replace '\\', '-' `
          -replace '/', '-' ).ToLower()
      $ScanName = $validatedOutputPath + "MigrationPlanningScan_" + $subscriptionName + "_" + $date + ".csv"
      $dependencyReportName = $validatedOutputPath + "DependencyReports_" + $subscriptionName + "_" + $date + ".csv"
